...
def getROIs(run):
    ...
    ret_dict = {}
    if run>0:
        # Alvium for Spatial timetool
        roi_dict3 = {}
        roi_dict3['ROIs'] = [None]
        #roi_dict3['ROIs'] =  [ [[440,700], [0,1935]] ]
        roi_dict3['writeArea'] = False
        roi_dict3['thresADU'] = None
        ret_dict['Alvium_u1'] = roi_dict3

        # EPIX100
        
        if run>300 and run<500:
            roi_dict = {}
            roi_dict['ROIs'] = [ [[0,704], [220,320]] ]
        ...
        #roi_dict2 = {}
        #roi_dict2['ROIs'] = [None]
        #roi_dict2['ROIs'] =  [ [[0,1], [50,150], [200,250]] ] # can define more than one ROI
        #roi_dict2['writeArea'] = False
        #roi_dict2['thresADU'] = None
        #ret_dict['epix10k2M'] = roi_dict2
    return ret_dict
...
def getAzIntParams(run):
    ...
    if run <130:
        az_dict['eBeam']= 8
        az_dict['center'] = [-983.979,242.433]
        az_dict['dis_to_sam']= 71.269
        az_dict['phiBins']=11
...
'''
    if run <999:
        az_dict['eBeam']= 8.
        az_dict['center'] = [-985.979,247.433]
        az_dict['dis_to_sam']= 60.00
        az_dict['phiBins']=11
        ret_dict['epix10k2M'] = az_dict

    else:
        az_dict['eBeam']= 7.1
        az_dict['center'] = [-985.4024,247.3123]
        az_dict['dis_to_sam']= 47.00
        az_dict['phiBins']=11
        ret_dict['epix10k2M'] = az_dict
'''
#
...
def define_dets(run):
    ...
    #detnames = ['epix10k2M', 'epix_1', 'Alvium_u1']
    detnames = ['Alvium_u1', 'epix_1', 'epix_2', 'epix10k2M']
    ...
    for detname in detnames:
        ...
        # Common mode
        if havedet:
            #if detname=='' :# or argss == 'libpressio': 
            if detname == 'epix10k2M':
                # change here to specify common mode for detname if desired. Else default is used
                common_mode=80
                #common_mode=None

                # print('**fake** not trying compression')
            else: 
                print('not doing compression',detname)
                common_mode=None

            if detname == 'epix10k2M':
                # change here to specify common mode for detname if desired. Else default is used
                if abs_error>0:
                  common_mode=90+(abs_error/10000) # encode the error in the mode
                  abs_error1 = (common_mode%100 - 90)*10000
                  print('doing compression common mode:', common_mode, abs_error1)
                  #abs_error1 = (common_mode%100 - 90)*10000
         
                else: 
                  common_mode = 80
                  print('not doing compression, common mode:', common_mode)
  


            det = DetObject(detname ,ds.env(), int(run), common_mode=common_mode, method = args.pretransform, AbsCError = abs_error )           
...
# import smalldata_tools.DetObject #import DetObject


#DetObject = smalldata_tools.DetObject.DetObject
...
parser.add_argument("--compressorE", type = float, help="compression abs error", default=0)
parser.add_argument("--pretransform", type = int, help="0: identity, 1: sqrt, 2: log", default=0)
...
###
import smalldata_tools.DetObject 
#DetObject = smalldata_tools.DetObject.DetObject

method = args.pretransform

## smalldata_tools.DetObject.set_pretransform(method)

setattr(smalldata_tools.DetObject, "method", method)

DetObject = smalldata_tools.DetObject.DetObject

print('det object method', smalldata_tools.DetObject.method )

abs_error = args.compressorE
if abs_error>0:
  print('absolute error bound:', abs_error)

print(f'compression abs error {abs_error}', file=open('output.log', 'w'))
